...
router.post('/register',async(req,res)=>{
     ...
     try{
        ...
        const result=await pool.query('insert into users(name,phone_number,email,password) values($1,$2,$3,$4) returning *',[name,phone_number,email,hashedPassword]);

        res.status(201).send({message:'User registered SUccessfully',user:result.rows[0]});
     }
     catch(err){
        console.error('Registration Error:', err);
        res.status(400).send({error:'User registration failed', details: err.message});
     }
});
